...
<div class="crm-container" lang="de" xml:lang="de" >
	<div class="crm-block crm-form-block crm-import-datasource-form-block">
		{if $status eq "closed"}
			...
			{if $allow_xml}
				<p><font size="+0.5">{ts}The money should be on its way{/ts}</font></p>
			{else}
		...
		{elseif $status eq "invalid"}
			...
			<ul>
				<li>{ts 1=$settings_url}Make sure your bank does support your XML PAIN format. If it doesn't, change the generated format on the <a href="%1">settings page</a>. Then dowload the file again.{/ts}</li>
		...
		{else}
			...
			{else}
				<p id="closed_group_instruction_text">
					{ts}<b>In order to collect these payments, you have to do the following:</b>{/ts}
					<ol>
						<li>{ts}Close this group with the option below{/ts}</li>
						<li>{ts}Download the XML file{/ts}</li>
						<li>{ts}Submit that file to your bank{/ts}</li>
					</ol>
					{ts}<b>Do it now! No money will be transferred until the file has been submitted successfully.</b>{/ts}
				</p>
	...
	{else}
		{if not $is_test_group}
			...
			{else}
				<a href="{crmURL p="civicrm/sepa/closegroup" q="group_id=$txgid&status=closed"}" class="button button_export">{ts}Close it now{/ts}</a>
			{/if}
		{/if}
...
var dontleave = "{ts}It's very important to select one of the options below.{/ts}";
