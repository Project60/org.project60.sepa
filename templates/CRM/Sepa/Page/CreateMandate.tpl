...
<form id='new_sepa_mandate' action="{$submit_url}" method="post">
	...
	<table>
		...
		<tr>	<!-- MANDATE REFERENCE -->
			...
			<td><input name="reference" type="text" size="34" value="{$reference}" placeholder="{ts escape='htmlattribute'}not required, will be generated{/ts}"/></td>
		</tr>
		<tr>	<!-- SOURCE -->
			<td>{ts}Source{/ts}:</td>
			<td><input name="source" type="text" value="{$source}" placeholder="{ts escape='htmlattribute'}not required{/ts}"/></td>
		</tr>
		<tr>	<!-- NOTE -->
			<td id="mandate_note_label">{ts}Note{/ts}:</td>
			<td><input name="note" type="text" size="32" value="{$note}" placeholder="{ts escape='htmlattribute'}not required{/ts}"/></td>
		</tr>
	...
	<input type="submit" value="{ts escape='htmlattribute'}create{/ts}" />
</form>
...
{else}
	...
	{/if}
	<br/><br/>
	<a href="{$back_url}" class="view button" title="{ts escape='htmlattribute'}back to contact{/ts}">
		<span><div class="icon preview-icon"></div>{ts}view contact{/ts}</span>
	</a>
	<a href="{$mandate_url}" class="view button" title="{ts escape='htmlattribute'}view mandate{/ts}">
