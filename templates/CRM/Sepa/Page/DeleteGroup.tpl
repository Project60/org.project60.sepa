...
<p>
	...
	{if $txgroup.status_name neq 'Open'}
		...
		{/if}
		<p class="status message"><i class="crm-i fa-exclamation-triangle" role="img" aria-hidden="true"></i>{ts}Only proceed if you know what you're doing.{/ts}</p>
	{/if}
</p>
...
<p>
	...
	<label for="delete_contents_OPEN">{ts 1=$entity_label}delete pending %1{/ts} ({ts 1=$entity_label}recommended{/ts})</label>
	</input>
	<br/>
</p>
...
<p id="warning_RCUR-Open_no" hidden="1" class="status message" name="warnings">
	...
	<br/>{ts}Deleting these contributions will <i>not</i> affect the associated mandates.{/ts}
</p>
...
<p id="warning_OOFF-Open_open" hidden="1" class="status message" name="warnings">
	...
	<br/>{ts}Deleting these mandates will also delete the associated contributions.{/ts}
</p>
...
<div class="crm-submit-buttons">
	<input id="ok_button" class="button button_close" type="button" value="{ts escape='htmlattribute'}Yes{/ts}" />
	<input id="cancel_button" class="button button_close" type="button" value="{ts escape='htmlattribute'}No{/ts}" />
</div>
</form>
