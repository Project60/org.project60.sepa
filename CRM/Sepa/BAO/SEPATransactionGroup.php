...
//and not cancelled
  ...
  public static function createFile($txgroup_id, $override = FALSE) {
    ...
    $creditor = civicrm_api3('SepaCreditor', 'getsingle', ['sequential' => 1, 'version' => 3, 'id' => $txgroup['sdd_creditor_id']]);
  ...
  public static function deleteGroup($txgroup_id, $delete_contributions_mode = 'no') {
    ...
    $result = civicrm_api3('SepaTransactionGroup', 'delete', ['id' => $txgroup_id, 'version' => 3]);
    if (empty($result['is_error'])) {
  ...
  public static function _deleteGroupContents($txgroup_id, $type, $selector) {
    ...
    if ($type == 'OOFF') {
      ...
      while ($results->fetch()) {
        ...
        if (empty($results->mandate_id)) {
          ...
          $delete = civicrm_api3('SepaMandate', 'delete', ['id' => $results->mandate_id, 'version' => 3]);
    ...
    while ($results->fetch()) {
      if (isset($deleted_contributions[$results->contribution_id])) {
        ...
        $delete = civicrm_api3('Contribution', 'delete', ['id' => $contribution_id, 'version' => 3]);
        if (empty($delete['is_error'])) {
